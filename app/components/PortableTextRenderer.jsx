		...
		<Highlight theme={themes.vsDark} code={code} language={language}>
			{({ className, style, tokens, getLineProps, getTokenProps }) => (
				<pre style={style} className="relative p-5">
					<CopyToClipboard
						onCopy={() => copied(true)}
						className={`absolute transition-all duration-200 ease-in-out right-2 top-2 ${
							isCopied ? "text-green-500" : "text-red-500"
						} bg-gray-300 px-1 py-1`}
						text={code}
					>
...
const components = {
	...
	marks: {
		...
		link: ({ value, children }) => {
			...
			return (
				<a
					className="no-underline text-purple-500 bg-gradient-to-r from-red-500 via-blue-400 to-lime-600 bg-[length:0%_2px] bg-no-repeat bg-left-bottom hover:bg-[length:100%_2px] transition-all duration-200"
					href={value?.href}
					target={target}
	...
	return (
		<ul className="flex gap-2 list-none p-0 m-0 py-2">
			{categories.map((category) => {
				return (
					<li className="text-xs bg-gray-300 px-4 py-2 uppercase">
						{category.title}
					</li>
				);
			})}
